...
const dummyUser = { name: "user", password: "password" };
...
async function seedDB() {
    try {
        ...
        let dummyAuthor = await User.findOne({ username: dummyUser.name });
        if (!dummyAuthor) {
        ...
        seeds.forEach(async (seed) => {
            // Create Trail & Comment
            const trail = await Trail.create(seed);
            const comment = await Comment.create({
                text: "This place is great, but I wish there was internet",
            });

            // Associate Dummy Author to Trail
            trail.author.id = dummyAuthor._id;
            trail.author.username = dummyAuthor.username;
